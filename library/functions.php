...
function getBookingRecords(){
	...
	$sql 	= "SELECT u.id AS uid, u.name, u.phone, u.email,
			   r.ucount, r.rdate, r.venue, r.duration, r.status, r.comments   
...
function getBTSRecords(){
	$per_page = 10;
	$page = (isset($_GET['page']) && $_GET['page'] != '') ? $_GET['page'] : 1;
	$start 	= ($page-1)*$per_page;
	$sql 	= "SELECT u.id AS uid, u.name, u.phone, u.email, sport_type, sportcat, tname, college, r.ucount, r.rdate, r.status, r.comments   
			   FROM tbl_users u, tbl_teamreg r 
			   WHERE u.id = r.uid  
			   ORDER BY r.id DESC LIMIT $start, $per_page";
	//echo $sql;
	$result = dbQuery($sql);
...
function getUserRecords(){
	...
	if($type == 'student') {
		...
		$sql = "SELECT  * FROM tbl_users u WHERE type != 'lecturer' AND id = $id ORDER BY u.id DESC";
	}
	else {
		$sql = "SELECT  * FROM tbl_users u WHERE type != 'lecturer' ORDER BY u.id DESC LIMIT $start, $per_page";
	}
...
function getSportList() {
	...
	$page 	= (isset($_GET['page']) && $_GET['page'] != '') ? $_GET['page'] : 1;
	$start 	= ($page-1)*$per_page;
	$sql 	= "SELECT * FROM tbl_sport ORDER BY id DESC LIMIT $start, $per_page";
	//echo $sql;
	$result = dbQuery($sql);
	$records = array();
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$records[] = array("sid" => $id,"ssport_type" => $sport_type, "sdate" => $date,"scategory" => $category, "svenue" => $venue,);	
	}//while
	return $records;
}
...
function getTeamList() {
	...
	$page 	= (isset($_GET['page']) && $_GET['page'] != '') ? $_GET['page'] : 1;
	$start 	= ($page-1)*$per_page;
	$sql 	= "SELECT * FROM tbl_teamreg ORDER BY id DESC LIMIT $start, $per_page";
	//echo $sql;
	$result = dbQuery($sql);
	$records = array();
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$records[] = array("tid" => $id,"tsport_type" => $sport_type, "tname" => $name, "tphone" => $phone,"temail" => $email, "tcollege" => $college, "ttname" => $tname, "tcategory" => $category, "ttlist" => $tlist,);	
	}//while
	return $records;
}

function generateSportPagination() {
	$per_page = 10;
	...
	for($i=1; $i<=$pages; $i++)	{
		$pageno .= "<li><a href=\"?v=SPORT&page=$i\">".$i."</a></li>";
	}
	$pageno .= 	"</ul>";
	return $pageno;
}
